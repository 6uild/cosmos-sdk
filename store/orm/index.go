...
var isIndexName = regexp.MustCompile(`^[a-z_]{3,20}$`).MatchString
...
// MultiRef contains a list of references to primary keys.
...
func NewMultiKeyIndex(name string, indexer MultiKeyIndexer, unique bool,
	refKey func([]byte) []byte) Index {
	if !isIndexName(name) {
		panic(fmt.Sprintf("Illegal index name: %q", name))
	}
...
func (i Index) Update(db KVStore, prev Object, save Object) error {
...
func (i Index) GetLike(db ReadOnlyKVStore, pattern Object) ([][]byte, error) {
...
func (i Index) GetAt(db ReadOnlyKVStore, index []byte) ([][]byte, error) {
...
func (i Index) GetPrefix(db ReadOnlyKVStore, prefix []byte) ([][]byte, error) {
...
func (i Index) move(db KVStore, prev Object, save Object) error {
...
func (i Index) remove(db KVStore, index []byte, pk []byte) error {
	...
	var data = new(MultiRef)
	if err := data.Unmarshal(cur); err != nil {
		return err
	}
	err = data.Remove(pk)
	if err != nil {
...
func (i Index) insert(db KVStore, index []byte, pk []byte) error {
	// don't deal with empty keys
	if len(index) == 0 {
		return nil
	}
