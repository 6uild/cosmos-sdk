...
//	DBKey(key []byte) []byte
//	Delete(db KVStore, key []byte) error
//	Get(db ReadOnlyKVStore, key []byte) (Object, error)
//	GetIndexed(db ReadOnlyKVStore, name string, key []byte) ([]Object, error)
//	GetIndexedLike(db ReadOnlyKVStore, name string, pattern Object) ([]Object, error)
//	Parse(key, value []byte) (Object, error)
//	Save(db KVStore, model Object) error
...
func NewBucket(cdc *codec.Codec, name string, emptyModel Model) bucket {
	if cdc == nil {
		panic("codec must be set")
	}
...
// Get one element
func (b bucket) Get(db ReadOnlyKVStore, key []byte) (Object, error) {
	dbkey := b.DBKey(key)
	bz, err := db.Get(dbkey)
	if err != nil {
		return nil, err
...
// Parse takes a key and value data (Model) and
...
func (b bucket) Parse(key, value []byte) (Object, error) {
	...
	if err := b.cdc.UnmarshalBinaryBare(value, entity); err != nil {
...
func (b bucket) Save(db KVStore, model Object) error {
	err := model.Validate()
	if err != nil {
		return err
	}

	bz, err := b.cdc.MarshalBinaryBare(model.Value())
	if err != nil {
		return errors.Wrap(err, "failed to amino encode")
	}
...
func (b bucket) Delete(db KVStore, key []byte) error {
...
func (b bucket) updateIndexes(db KVStore, key []byte, model Object) error {
	// update all indexes
	if len(b.indexes) > 0 {
...
func (b bucket) GetIndexed(db ReadOnlyKVStore, name string, key []byte) ([]Object, error) {
	idx := b.indexes.Get(name)
	if idx == nil {
		return nil, errors.Wrap(errors.ErrInvalidIndex, name)
	}
...
func (b bucket) GetIndexedLike(db ReadOnlyKVStore, name string, pattern Object) ([]Object, error) {
	idx := b.indexes.Get(name)
	if idx == nil {
		return nil, errors.Wrap(errors.ErrInvalidIndex, name)
	}
...
func (b bucket) readRefs(db ReadOnlyKVStore, refs [][]byte) ([]Object, error) {
	if len(refs) == 0 {
		return nil, nil
	}
