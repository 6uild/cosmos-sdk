...
type ModelBucket interface {
	...
	// is returned.
	One(db ReadOnlyKVStore, key []byte, dest Model) error
	...
	// modified.
	ByIndex(db ReadOnlyKVStore, indexName string, key []byte, dest ModelSlicePtr) (keys [][]byte, err error)
	...
	// be overwritten.
	Put(db KVStore, key []byte, m Model) ([]byte, error)
	...
	Has(db ReadOnlyKVStore, key []byte) error
}
...
// KVStore instead.
func NewModelBucket(cdc *codec.Codec, name string, m Model, opts ...ModelBucketOption) ModelBucket {
	b := NewBucket(cdc, name, m)
...
func (mb *modelBucket) One(db ReadOnlyKVStore, key []byte, dest Model) error {
...
func (mb *modelBucket) ByIndex(db ReadOnlyKVStore, indexName string, key []byte, destination ModelSlicePtr) ([][]byte, error) {
...
func (mb *modelBucket) Put(db KVStore, key []byte, m Model) ([]byte, error) {
...
func (mb *modelBucket) Delete(db KVStore, key []byte) error {
...
func (mb *modelBucket) Has(db ReadOnlyKVStore, key []byte) error {
	if key == nil {
